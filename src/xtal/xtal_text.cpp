...
void initialize_text(){
	...
	tm->set_at("Xtal Runtime Error 1001", "%(object)s 関数呼び出しの '%(no)s'番目の引数の型が不正です。");
	...
	tm->set_at("Xtal Runtime Error 1005", "'%(object)s' 関数呼び出しの引数の数が不正です。%(min)s以上の引数を受け取る関数に、%(value)s個の引数を渡しました。");
	tm->set_at("Xtal Runtime Error 1006", "'%(object)s' 関数呼び出しの引数の数が不正です。%(min)s以上、%(max)s以下の引数を受け取る関数に、%(value)s個の引数を渡しました。");
	tm->set_at("Xtal Runtime Error 1007", "'%(object)s' 関数呼び出しの引数の数が不正です。引数を取らない関数に、%(value)s個の引数を渡しました。");
	tm->set_at("Xtal Runtime Error 1008", "'%(object)s'はシリアライズできません。");
	tm->set_at("Xtal Runtime Error 1009", "不正なコンパイル済みXtalファイルです。");
	tm->set_at("Xtal Runtime Error 1010", "コンパイルエラーが発生しました。");
	tm->set_at("Xtal Runtime Error 1011", "%(object)sは既に定義されています。");
	...
	tm->set_at("Xtal Runtime Error 1015", "%(object)s は定義されていません。");
	tm->set_at("Xtal Runtime Error 1016", "ファイル '%(name)s' のコンパイル中、コンパイルエラーが発生しました。");
	tm->set_at("Xtal Runtime Error 1017", "%(object)s は %(accessibility)s です。");
